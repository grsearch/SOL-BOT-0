...
const RSI_CACHE_TTL_MS = 60_000;       // 缓存 60 秒
const AUTO_BUY_COOLDOWN_MS = 60_000;   // 同一币自动买入失败后冷却 60 秒
...
class StrategyEngine {
  ...
  private isProcessingPrice = new Set<string>();   // 互斥
  private isAutoBuying = new Set<string>();        // 自动买入互斥
  private rsiCache = new Map<string, RsiCache>();
  private autoBuyBackoff = new Map<string, number>();  // 自动买入失败后退避
  ...
  private async runAutoStrategySweep(): Promise<void> {
    ...
    for (const t of tokens) {
      try {
        // 强制刷新这个币的 RSI（让 onPriceTick 也能读到新值）
        await this.refreshRsi7(t.address);
        // 评估自动买入
        await this.evaluateAutoDipBuy(t);
      } catch (e: any) {
        logger.warn({ err: e.message, addr: t.address }, '自动策略 sweep 单币失败');
      }
    }
  }
